...
<div class="btn-container">
	<button class="btn-gray"><a href="ecom-cart-3.php">Retail Cart</a></button>
	<button class="btn-blue"><a href="ecom-cart-wholesale.php">Wholesale Cart</a></button>
</div>
...
if (isset($_SESSION['usersuid'])){
	...
	$select_products = $cart_query = "SELECT * FROM `wholesale_cart` WHERE userId = '$usersid' ORDER BY selected_price DESC";
...
<?php echo "<a href='ecom-cart-delete.php?wholesale_id=$product_id'><i class='fa fa-trash-alt cart-remove'></i></a>"; ?>
</div>
</div>
</div>
<?php 
...
?>
	...
	<div class='total'>
		...
		<?php if($total < 2000){ ?>
			<div class='btn'>
			<button type='button' class='btn-checkout' style="background-color: gray;" onclick="return alert('Minimum total amount for wholesale is P2,000.')">Checkout</button>
			</div>
		<?php }
			if ($total >= 2000){ ?>
			<div class='btn'>
				<button type='button' class='btn-checkout'><a href='ecom-wholesale-checkout.php'>Checkout</a></button>
			</div>
		<?php } ?>
	</div></div>"; 
	<?php
