...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/delivered-orders.scss">
	<link rel="icon" type="image/x-icon" href="images/PlaceholderFavIcon.ico">
	<link href='https://fonts.googleapis.com/css?family=Poppins' rel='stylesheet'>
...
if (isset($_SESSION['usersuid'])){
//Pending Order Items
if(isset($_GET['completed-order'])){
    $item_order_id = mysqli_escape_string($link, $_GET['completed-order']);
    $user_id = $_SESSION['usersid'];

    $select_user_order = "SELECT * FROM `completed_orders` o, `users` u WHERE o.user_id = u.usersId AND order_id=$item_order_id";
    ...
    $date_format  = date("d-M-Y (h:i A)", strtotime($run_user_order['delivered_date']));
    ?>
    
    <!-- Nav Bar -->
    <nav class="your-cart">
        <div class="cart-header">
            <a href="ecom-delivered-orders.php?completed-order-list=<?php echo $user_id?>"><i class="fa-solid fa-arrow-left alt back-arrow"></i></a>
    ...
    $select_order_prods = "SELECT * FROM `completed_orders` o, `completed_order_products` oi WHERE o.order_id=$item_order_id AND oi.order_id=$item_order_id";
...
if(isset($_GET['completed-order-list'])){
    $item_order_id = mysqli_escape_string($link, $_GET['completed-order-list']);
    $user_completed_id = $_SESSION['usersid'];
    $select_completed = "SELECT * FROM `completed_orders` WHERE user_id=$user_completed_id";
    $run_completed = mysqli_query($link, $select_completed);
    if(mysqli_num_rows($run_completed)>0){
?>
<!-- Nav Bar -->
<nav class="your-cart">
    <div class="cart-header">
        <a href="home.php"><i class="fa-solid fa-arrow-left alt back-arrow"></i></a>
        <h4 class="cart-h4">Your Completed Orders</h4>
    </div>
</nav>
<!-- Pending and Delivered Buttons -->
<div class="container-fluid">
<div class="btn-container">
    <button class="btn-pending"><a href="ecom-pending-orders.php?pending-order-list=<?=$user_completed_id?>">Pending</a></button>
    <button class="btn-delivered"><a href="ecom-delivered-orders.php?completed-order-list=<?=$user_completed_id?>">Delivered</a></button>
</div>
<div class="order-completed">
        <div class="order-text">
            <h5>Completed Orders</h5>
            <p>Thanks for Shopping with Bulacan Pet Fish Supply!</p>
        </div>
...
<div class="container">
    <div class="d-flex justify-content-between align-items-center py-3">
        <h2 class="h5 mb-0"><a href="#" class="text-muted"></a>Your Completed Orders </h2>
    </div>
</div>

<?php 
    $pending_order_user_id = mysqli_real_escape_string($link, $_GET['completed-order-list']);
    $select_order = "SELECT * FROM `completed_orders` WHERE user_id = $pending_order_user_id";
    $run_order = mysqli_query($link, $select_order);
    while($row_order = mysqli_fetch_assoc($run_order)){ 
        $order_date_format = date("d-M-Y (h:i A)", strtotime($row_order['delivered_date'])); ?>
            ...
            <div class="row">
                ...
                <div class="card mb-10">
                    ...
                    <table class="table">
                        ...
                        <tr class="fw-bold">
                            <td colspan="2"></td>
                            <td class="text-end"><a href="ecom-delivered-orders.php?completed-order=<?php echo $row_order['order_id']?>"><button style='background-color: #000C1D; color: white;'>View Order</button></a></td>
                        </tr>
                        </tfoot>
                    </table>
                    </div>
...
<nav class="your-cart">
    <div class="cart-header">
        <a href="home.php"><i class="fa-solid fa-arrow-left alt back-arrow"></i></a>
        <h4 class="cart-h4">My Completed Orders</h4>
    </div>
</nav>
<!-- Pending and Delivered Buttons -->
<div class="container-fluid">
