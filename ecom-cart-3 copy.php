...
if (isset($_SESSION['usersuid'])){
	...
	$select_products = $cart_query = "SELECT * FROM `cart_details` WHERE `ip_address`='$get_ip' AND userId = '$usersid' ORDER BY selected_price DESC";
...
<div class='cart-box'>
	...
	<img src='./Product Images/$image' alt='' class='cart-img'>
	</div>
<div class='detail-box'>
<!-- Product Title -->
<div class='cart-product-title'>$product_title</div>
<!-- Product Price -->
<div class='cart-price'>$selected_price.00'</div>
<div class='cart-quantity-container'>
	<!-- Incr/Decr Quantity FRONTEND -->
	<input type='submit' name='new-quantity' value = 'change'>
	<p>Qty: </p>
	<input type='text' name='enter-quantity[]' value='$quantity' class='cart-quantity' onChange='this.form.submit()' min='1''>
	<input type='hidden' name='product-ids[]' value='$product_id'>
</div> 
</div>
<!-- Remove from cart -->
<div class='cart-remove-btn'>
<a href='ecom-cart-delete.php?prod_id=$product_id'><i class='fa fa-trash-alt cart-remove'></i></a>
</div>
</div>
</div>"
?>
